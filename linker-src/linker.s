...
write_machine_code_next_inst:
	...
	jal parse_int
	move $s6, $v0 # $s6 contains the instruction as a number.
	
	
	# 4. Check if the instruction needs relocation. If it does not, branch to
	# the label write_machine_code_to_file:
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $s6 #the instruction
	jal inst_needs_relocation
	beq $v0, 0, write_machine_code_to_file # 0 means instruction does not need relocation.
	...
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $s6 #  $a0 = an instruction that needs relocating
	move $a1, $s5 #  $a1 = the byte offset of the instruction in the current file
	move $a2, $s2 #  $a2 = the symbol table
	move $a3, $s3 #  $a3 = the relocation table
	jal relocate_inst
	li $t0, -1
	beq $v0, $t0, write_machine_code_error # -1 if error
	move $s6, $v0
...
write_machine_code_to_file:
	...
	move $a0, $s6 #  $a0 = int to write
	la $a1, hex_buffer #  $a1 = character buffer to write into
	jal hex_to_str
